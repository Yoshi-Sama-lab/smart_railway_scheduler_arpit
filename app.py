...
class APIConfig:
    ...
    OPENWEATHER_API_KEY = "chori mat karna"  
    WEATHERAPI_KEY = "chori mat karna"       
    
    
    RAPIDAPI_KEY = "chori mat karna"          
...
class SmartRailwaySystem:
    ...
    def _add_complete_intelligence(self, train_dict):
        ...
        try:
            ...
            recommendation = self._get_smart_recommendation(predicted_delay, score, weather_data)
            live_data = None
    ...
    def _calculate_optimization_score(self, train_type, weather_data, predicted_delay):
        ...
        base_scores = {
            'RAJDHANI': 0.88, 'SHATABDI': 0.88, 'DURONTO': 0.82,
            'SUPERFAST': 0.78, 'EXPRESS': 0.68, 'MAIL': 0.65
        }
        
        score = base_scores.get(train_type, 0.68)
        
        
        weather_multipliers = {
            'Clear': 1.0, 'Sunny': 1.0, 'Pleasant': 1.0,
            'Cloudy': 0.98, 'Haze': 0.95,
            'Rain': 0.9, 'Heavy Rain': 0.8,
            'Fog': 0.7, 'Thunderstorm': 0.75
        }
        
        weather_factor = weather_multipliers.get(weather_data['condition'], 1.0)
        score *= weather_factor
        
        
        score *= max(0.5, 1 - predicted_delay / 80)
        
        return score
    
    def _get_smart_recommendation(self, delay, score, weather_data):
        """Get intelligent recommendation"""
        
        severe_weather = weather_data['condition'] in ['Heavy Rain', 'Fog', 'Thunderstorm']
        
        if severe_weather:
            if delay <= 15 and score > 0.75:
                return 'RECOMMENDED'
            elif delay <= 25 and score > 0.6:
                return 'CONSIDER'
            else:
                return 'CAUTION'
        else:
            if delay <= 10 and score > 0.8:
                return 'HIGHLY_RECOMMENDED'
            elif delay <= 20 and score > 0.7:
                return 'RECOMMENDED'
            elif delay <= 35:
                return 'CONSIDER'
            else:
